...
const BatchesPage = () => {
  ...
  if (selectedBatchId) {
    ...
    return <BatchDetailView batch={batch} onBack={() => setSelectedBatchId(null)} />;
  }
...
const BatchDetailView = ({ batch, onBack }) => {
  const { user } = useContext(AuthContext);
  ...
  const loadOperations = async () => {
    try {
      setLoading(true);
      const data = await api.get(`/batches/${batch.id}/operations`);
  ...
  const handleCreateStage = async (e, stageId) => {
    ...
    try {
      ...
      const msg = err.message || 'Ошибка при создании этапа';
      setError(msg);
    }
  };

  const getStageCount = (stageId) => {
    return operations.filter(op => op.stage === stageId).length;
  };

  const getStageQuantity = (stageId) => {
    return operations
      .filter(op => op.stage === stageId)
      .reduce((sum, op) => sum + (op.quantity_processed || 0), 0);
  };
      ...
      <div className="card mb-4">
        <div className="card-header">
          ...
          <span
            className="badge"
            style={{
              backgroundColor: BATCH_STATUS_MAP[batch.status]?.color + '30',
              color: BATCH_STATUS_MAP[batch.status]?.color,
              fontSize: '0.875rem',
              padding: '0.5rem 1rem'
            }}
          >
            {BATCH_STATUS_MAP[batch.status]?.label}
          </span>
        </div>
        <div className="card-body">
            ...
            {PRODUCTION_STEPS.map((step) => (
              <div key={step.id} style={{
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.75rem' }}>
                  ...
                  <div style={{ fontSize: '0.875rem', color: '#94a3b8' }}>
                    {getStageCount(step.id)} операций • {getStageQuantity(step.id)} шт.
                  </div>
                </div>
                ...
                {getStageCount(step.id) > 0 && (
                  <div style={{ marginBottom: '0.75rem', fontSize: '0.875rem', color: '#64748b' }}>
                    {operations
                      .filter(op => op.stage === step.id)
                      .map((op, idx) => (
                        <div key={op.id || idx} style={{ paddingLeft: '2rem', marginBottom: '0.25rem' }}>
                          • {op.quantity_processed} шт. {op.created_at && `— ${new Date(op.created_at).toLocaleDateString('ru')}`}
                        </div>
                      ))
                    }
                  </div>
                ...
                {showStageForm === step.id ? (
                  <form onSubmit={(e) => handleCreateStage(e, step.id)} style={{ marginTop: '0.75rem' }}>
                    ...
                    <div style={{ display: 'flex', gap: '0.5rem' }}>
                      ...
                      <button type="button" className="btn btn-sm btn-secondary" onClick={() => setShowStageForm(null)}>
                        Отмена
                      </button>
                    </div>
                  </form>
